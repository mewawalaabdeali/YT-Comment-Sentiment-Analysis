...
def get_root_directory() -> str:
    """Get the root directory (two levels up from this scripts location)"""
...
def apply_tfidf(train_data:pd.DataFrame, max_features:int, ngram_range:tuple) -> tuple:
    ...
    try:
        ...
        logger.debug(f"TF-IDF transformation commplete. Train shape: {X_train_tfidf.shape}")
...
def train_lgbm(X_train: np.ndarray, y_train:np.ndarray, learning_rate:float, max_depth: int, n_estimators:int)-> lgb.LGBMClassifier:
    ...
    try:
        ...
        logger.debug('LightGBM model training commpleted')
        return best_model
    except Exception as e:
        logger.error('Error during LighGBM model training: %s',e)
        raise
...
def main():
    try:
        ...
        #Load the processed training data from the interim directory
        train_data = load_data(os.path.join(root_dir, 'data/interim/train_processed.csv'))

        #Apply TfIdf feature engineering on training data
        ...
        logger.error('Failed to complete the feature engineering nad model building process: %s', e)
        print(f"Error: {e}")
